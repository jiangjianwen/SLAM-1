...
int main(int argc, char **argv) {
    ...
    Mat ref_depth;
    bool ret = readDataseFiles(argv[1], color_image_files, poses_TWC, ref_depth);
    if (ret == false) {
...
bool readDataseFiles(
    ...
    if (!fin)
        return false;
    while(!fin.eof()){
        // 数据格式: 图像文件名 tx, ty, tz, qx, qy, qz, qw, 注意是 TWC 而不是 TCW
        string image;
        fin >> image;
        double data[7];
        for (double &d : data)
...
bool epipolarSearch(
        ...
        Vector2d &pt_curr,
        Vector2d &epipolar_direction){
    Vector3d f_ref = px2cam(pt_ref); // 得到参考点的像素转换到相机坐标系下的坐标
    f_ref.normalize();
    ...
    if (half_length > 100) half_length = 100;            // 不希望搜索太多
