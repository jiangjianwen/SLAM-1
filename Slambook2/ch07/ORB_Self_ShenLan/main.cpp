/*
 * 本程序主要完成视觉slam理论与实践第五课后习题二部分 ORB特证
 * */
 ...
 */
 // KeyPoint各项属性如下:
 // pt(x,y):关键点的坐标, size(): 该关键点邻域直径大小, angle:角度,表示关键点的方向,值为[0,360],负值表示不使用. response响应强度
 // octacv:从哪一层金字塔得到的关键点. class_id 当要对图像进行分类时,用class_id对每个关键点进行分析,默认值为-1.
 ...
 */
using DescType = std::vector<bool>;
void computeORBDesc(const cv::Mat &image, std::vector<cv::KeyPoint> &keypoints, std::vector<DescType> &desc);
...
int main(int argc, char ** argv) {
    ...
    std::cout << "keypoints: " << keypoints2.size() << std::endl;

    // compute angle for each keypoint
    computeAngle(second_image, keypoints2);

    // compute ORB descriptors
    std::vector<DescType> descriptors2;
...
// -------------------------------------------------------------------------------------------------- //
// ORB pattern 也就是计算描述子的时候p,q的取法 需要按照固定的pattern来计算描述子否则的话将会导致描述子不稳定
...
void computeAngle(const cv::Mat &image, std::vector<cv::KeyPoint> &keypoints){
    ...
    for (auto &kp : keypoints){
        ...
        double m01 = 0, m10 = 0;    // 定义变量的时候,需要初始化,不然这里第一张图片所有kp.angle = 0
        // 获取每个图像块中像素点的像素值 -8<i<8, -8<j<8
        for (int i = -half_patch_size; i < half_patch_size; i++){
            for (int j = -half_patch_size; j < half_patch_size; j++){
                // 注意 x表示的是图像的列, y表示的是图像的行 每个像素的真实坐标是(j, i) + (y, x)
                // 获取单个像素值image.at<uchar>(y,x)
                m01 += j*image.at<uchar>(y+j, x+i);
                m10 += i*image.at<uchar>(y+j, x+i);
            }
        }
        // 由于OpenCV库使用的是角度制 atan返回的是弧度制, 因此需要转换成角度制
        // 角度 = 弧度x180°/π
        kp.angle = std::atan(m01/m10)*180/pi;
        std::cout << "m01 = " << m01 << "  " << "m10 = " << m10 << "  " << "kp.angle = " << kp.angle << std::endl;
    }
    return;
}
...
// 计算描述子
// 为了让描述子面对图像旋转时,仍然有效!
// 先让p, q按照keypoint的角度进行旋转,使得图像发生旋转以后,p与q的位置相对于keypoint不发生旋转,然后再进行像素大小比较,再进行keypoint的匹配
void computeORBDesc(const cv::Mat &image, std::vector<cv::KeyPoint> &keypoints, std::vector<DescType> &desc){
    for (auto& kp : keypoints){
        DescType d(256, false);
        for (int i =  0; i < 256; i++){
            // OpenCV存储的角度是角度制首先转换成弧度制之后在进行计算
            // 弧度=角度xπ/180°
            auto cos_ = static_cast<float>(cos(kp.angle*pi/180));
            auto sin_ = static_cast<float>(sin(kp.angle*pi/180));
            // 注意pattern中的数据如何取, 计算p和q 先计算坐标在计算像素值
            // p_r,q_r计算的只是距离中心点所偏离的距离
...
void bfMatch(const std::vector<DescType> &desc1, const std::vector<DescType> &desc2, std::vector<cv::DMatch> &matches){
    // 设定一个距离阀值即认为匹配的特征点距离不应该大于dmax
    int d_max = 50;
    for (int i = 0; i < desc1.size(); i++){
        if(desc1[i].empty())
            continue;
        int d_min = 256, index = 1; // 每次循环需要重新初始化
        for (int j = 0; j < desc2.size(); j++){
            // 取出desc2中的每一个描述子和desc[i]计算距离, 取最小的距离作为最后的结果
            if (desc2.empty())
                continue;
            int d = 0;  // 每次循环需要重新进行初始化
            for (int k = 0; k < 256; k++){
                d += desc1[i][k]^desc2[j][k]; // 对每一位进行异或操作,不同的为1
            }
            if(d < d_min){
                d_min = d;
                index = j;
