/*
 * 本示例主要完成利用opencv, 高斯牛顿, g2o来最小化重投影误差
 * */
#include <iostream>
...
int main(int argc, char ** argv) {
    ...
    // 建立3D点
    Mat d1 = imread(argv[3], IMREAD_UNCHANGED); // 深度图为16位无符号整数, 单通道图像
    ...
    Mat R;
    cv::Rodrigues(r, R); // r为旋转向量形式，用Rodrigues罗德里得斯公式转换为矩阵
...
void find_feature_matches(
    ...
    // 使用OpenCV3 创建描述子
    Ptr<FeatureDetector> dector = ORB::create();
    ...
    printf("--Max dist: %f \n", max_dist);
    printf("--Min dist: %f \n", min_dist);
    // 当描述子之间的距离大于两倍的最小距离时, 即认为匹配有误, 但是有时候最小距离会非常小, 设置一个经验值30作为下限
    for (int i = 0; i < descriptors_1.rows; i++){
...
void bundleAdjustmentGaussNewton(
    ...
    for (int iter = 0; iter < iterations; iter++){
        ...
        for (int i = 0; i < points_3d.size(); i++){
            Eigen::Vector3d pc = pose * points_3d[i]; // 利用变换矩阵 将相机1位姿的空间点变换到相机2的坐标系下面
            double inv_z = 1.0 / pc[2];
            double inv_z2 = inv_z * inv_z;
            // 利用相机模型,将坐标转换到像素坐标
            Eigen::Vector2d proj(fx * pc[0] / pc[2] + cx, fy * pc[2] + cy);
            // 计算实际图像二中的像素坐标与估计出来的像素坐标之间的误差
...
/// vertex顶点(第二个相机的位姿) and edges边(每个3D点在第二个相机中的投影) used in g2o ba
...
void bundleAdjustmentG2O(
    ...
    // vertex
    VertexPose *vertexPose = new VertexPose();  // 相机顶点位姿
    vertexPose->setId(0);
    ...
    chrono::steady_clock::time_point t2 = chrono::steady_clock::now();
    chrono::duration<double> time_used = chrono::duration_cast<chrono::duration<double>>(t2 - t1);
    cout << "optimization costs time: " << time_used.count() << " seconnds. " << endl;
