...
void ComputeORB(const cv::Mat & img, std::vector<cv::KeyPoint>& keypoints, std::vector<DescType>& descriptors){
    ...
    for (auto &kp : keypoints){
        ...
        float m_sqrt = std::sqrt(m01 * m01 + m10 * m10) + 1e-18;    // 避免除数为0
        float sin_theta = m01 / m_sqrt;
        float cos_theta = m10 / m_sqrt;

        // compute the angle of this point
        DescType desc(8, 0); // 使用8个字节存储信息,共计256位,初始值初始化为0
        for (int i = 0; i < 8; i++) {
            uint32_t d = 0;
            for (int k = 0; k < 32; k++) {
                int idx_pq = i * 8 + k;
...
/*
 * 暴力搜索思路很简单. 给定两组描述子P=[p1,....pm]和Q=[q1,....qn].
 * 那么, 对于P中任意一个点,找到Q中对应最小距离点,即算一次匹配.但是这样做会对每个特征点都找到一个匹配,所以
 * 通常需要限制一个距离阀值dmax, 即认为匹配的特征点距离步应该大于dmax.
 * */
void BfMatch(const std::vector<DescType> &desc1, const std::vector<DescType> &desc2, std::vector<cv::DMatch> &matches) {
    const int d_max = 40;
    ...
    for (size_t i1 = 0; i1 < desc1.size(); ++i1) {
        ...
        for (size_t i2 = 0; i2 < desc2.size(); ++i2) {
            ...
            int distance = 0;
            for (int k = 0; k < 8; k++) {
                distance += _mm_popcnt_u32(desc1[i1][k] ^ desc2[i2][k]);  // _mm_popcnt_u32函数计算变量中1的个数 找出多少位不一样
            }
            if (distance < d_max && distance < m.distance) {
                m.distance = distance;
                m.trainIdx = i2;  // 第二个图的特征id
            }
        }
        if (m.distance < d_max) {
            matches.push_back(m);
