...
public:
    ...
    void championLaserScanCallback(const champion_nav_msgs::ChampionNavLaserScanConstPtr& msg)
    {
        ...
        if(m_isFirstFrame == true)
        {
            ...
            ConvertChampionLaserScanToEigenPointCloud(msg, m_prevPointCloud);  // 将信息格式转换为imls匹配的Eigen库形式
            return ;
        }

        std::vector<Eigen::Vector2d> nowPts;
        ConvertChampionLaserScanToEigenPointCloud(msg, nowPts);
        // 把当前帧的点云和上一帧的点云同时传递给imls进行匹配
        //调用imls进行icp匹配，并输出结果．
        m_imlsMatcher.setSourcePointCloud(nowPts);
        m_imlsMatcher.setTargetPointCloud(m_prevPointCloud);

        Eigen::Matrix3d rPose,rCovariance;  // rPose表示得到两帧激光之间的位姿增量, rCovariance保存协方差矩阵
        if(m_imlsMatcher.Match(rPose,rCovariance))
        {
            ...
            Eigen::Matrix3d lastPose;  // 上一帧激光雷达的位姿
            lastPose << cos(m_prevLaserPose(2)), -sin(m_prevLaserPose(2)), m_prevLaserPose(0),
                        ...
                        0, 0, 1;
            Eigen::Matrix3d nowPose = lastPose * rPose; // 上一帧的位姿*位姿增量得到当前帧的位姿
        ...
        m_prevPointCloud = nowPts; // 把当前帧的点云赋给上一帧点云
    }
    // 里程计的消息回调函数只是为了发布路径
    ...
    void pubPath(Eigen::Vector3d& pose, nav_msgs::Path &path, ros::Publisher &mcu_path_pub_)
    {
        ...
        // yaw为绕z轴的旋转角度, 因此从ros中构造四元数赋值给位姿
        ...
        this_pose_stamped.header.stamp = current_time;  // 时间戳
        this_pose_stamped.header.frame_id = "odom";     // 设置坐标系
    ...
    bool m_isFirstFrame;  // 判断是否是第一帧激光消息
    // ros::NodeHandle m_nh;
