...
//IMLS函数，主要用来进行曲面投影．
//可以认为是xi在曲面上的高度． 计算in_cloud中的点到target_cloud构成的表面上的高度差
                                         ...
                                         double& height)
{
    ...
    for(int i = 0; i < searchNumber;i++)
    {
                ...
                std::isnan(nearDist2(i)) == false)
        {
            ...
            Eigen::Vector2d tmpPt(m_targetKDTreeDataBase(0,index),m_targetKDTreeDataBase(1,index)); // 取出目标点云中的数据
    ...
    //如果nearPoints小于３个，则认为没有匹配点．这里只是为了增加程序的健壮性, 已经在多个地方进行判断了
    ...
    //TODO
    //根据函数进行投影．计算height，即论文中的I(x)公式1
    double mh2 = m_h * m_h;
    ...
    height = projSum / (weightSum + 0.00000001);
    //end of TODO
        ...
        std::vector<Eigen::Vector2d> &out_normal)
{
    ...
    for(std::vector<Eigen::Vector2d>::iterator it = in_cloud.begin(); it != in_cloud.end();)
    {
        ...
        m_pTargetKDTree->knn(xi,indices,dist2); // 在目标点云中找到距离in_cloud(源点云)最近的点, 用这个点的法向量近似为投影点的法向量
        ...
        {
            it = in_cloud.erase(it); // 删除找不到对应的法向量的匹配点
            continue;
        }
        ...
        //end of TODO
        out_cloud.push_back(yi);     // 存储对应的投影点以及投影点所对应的法向量
                                                  ...
                                                  Eigen::Matrix3d& deltaTrans)
{
    ...
    for(int i = 0; i < source_cloud.size();i++)
    {
        ...
        Mi <<   1,0,p(0),-p(1),
                0,1,p(1), p(0);
        M += Mi.transpose() * Ci * Mi; // 维度[4,4]

        Eigen::Matrix<double,1,4> gti;
        gti  = -2 * refPt.transpose() * Ci * Mi; // 维度 [1,4]
    ...
    SA = S.determinant() * S.inverse(); // S.determinant()求矩阵的行列式
...
Eigen::Vector2d IMLSICPMatcher::ComputeNormal(std::vector<Eigen::Vector2d> &nearPoints)
{
    ...
    // 临近点的数量小于3, 无法正常计算法向量直接赋值为inf也就是不合法的法向量, 会在后续的匹配中直接去除掉
    if(nearPoints.size() < 3){
        normal(0) = normal(1) = std::numeric_limits<double>::infinity();
        return normal;
    }

    Eigen::Vector2d centroid(0, 0); // 当前临近点的质心
    // 累加附近最近点
    for (auto point : nearPoints){
        centroid += point;
    }
    centroid /= static_cast<double>(nearPoints.size()); // 平均值即为质心坐标

    Eigen::Matrix2d sigma = Eigen::Matrix2d::Zero();  // 存储协方差矩阵
    for (auto point : nearPoints){
        Eigen::Vector2d temp = point - centroid;  // 计算到执行的距离
        sigma += temp * temp.transpose();         // 累加协方差矩阵
    }
    sigma /= static_cast<double>(nearPoints.size());  // 取平均, 避免上面累加的结果过大

    // 计算法向量, 第一个特征向量即为最小特征值对应的特征向量, 矩阵的第一列
    Eigen::SelfAdjointEigenSolver<Eigen::Matrix2d> eig(sigma);
    Eigen::Matrix2d eigVectors = eig.eigenvectors(); // 这里特征值是按照从大到小的方式进行排列的, 最小特征值排在前面, 最小特征值对应的特征向量就是法向量, 理论可以参考pcl从入门到精通
 ...
 * 最终使用的ICP匹配函数．计算两帧激光之间的匹配位姿
 * @param finalResult
 * @param covariance  保存协方差矩阵
 * @return
 */
                           ...
                           Eigen::Matrix3d& covariance)
{
    ...
    if(m_isGetNormals == false)
    {
        ...
        for(int i = 0; i < m_targetPointCloud.size();i++)
        {
            ...
            // 每个Target点云取出离它最近相距在0.15m范围内的20个点
            int K = 20;
            Eigen::VectorXi indices(K);  // 存储最近的20个点的index
            Eigen::VectorXd dist2(K);    // 存储最近的20个点距离当前点的distance
            // 使用knn()函数查找最近点, epsilon表示精确搜索
            int num = m_pTargetKDTree->knn(xi,indices,dist2,K,0.0,
                                           Nabo::NNSearchD::SORT_RESULTS | Nabo::NNSearchD::ALLOW_SELF_MATCH|
                                           Nabo::NNSearchD::TOUCH_STATISTICS,
                                           0.15);

            std::vector<Eigen::Vector2d> nearPoints;  // 存储当前点的最近点
            ...
            {
                normal(0) = normal(1) = std::numeric_limits<double>::infinity(); // 最近点数量小于3法向量直接赋值为inf标记为不合法的数据方便剔除
            }
            m_targetPtCloudNormals.push_back(normal); // 把法向量存储起来方便之后的调用
        }
    }
    ...
    for(int i = 0; i < m_Iterations;i++)
    {
        ...
        for(int ix = 0; ix < m_sourcePointCloud.size();ix++)
        {
            ...
            origin_pt << m_sourcePointCloud[ix],1; // 这里使用齐次坐标方便进行运算

            Eigen::Vector3d now_pt = result * origin_pt;  // [3,3]*[3,1] ==> [3,1]
        ...
        std::vector<Eigen::Vector2d> ref_cloud;  // in_cloud上的点在上一帧点云构成的平面上的投影点存储在ref_cloud中也就是y
        ...
        Eigen::Matrix3d deltaTrans;  // 保存位姿的增量
        // 二维场景下就是求解点线ICP, 方法参考ICP的论文An_ICP_variant_using_a_point-to-line_metric
